...
correct or push back on (for example the two real bugs found only once the
tests ran on your machine — the `attachments.ts` router-scoping bug and the
`.env` loading gap — and the two test-only bugs found afterward), and one
thing this process taught you about trusting "the code looks right" versus
"the tests actually ran".]_
